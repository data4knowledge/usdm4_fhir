...
def create_breadcrumb(current_page, section_title=None, element_name=None):
    ...
    if element_name:
        ...
        breadcrumb += f'<li class="breadcrumb-item active" aria-current="page">Section Overview</li>'
    else:
...
def create_index_page(data, html_dir):
    ...
    print(f"Created index.html with {total_sections} sections and {total_elements} elements")
...
def create_section_page(section_title, elements, html_dir):
    ...
    for element_name, element_data in sorted_elements:
        ...
        element_filename = element_name.replace(' ', '_').replace('/', '_').replace('?', '').replace(':', '')
...
def create_element_page(element_name, element_data, section_title, html_dir):
    ...
    # Template panel
    if template_data:
        content += create_data_panel("Template Information", template_data, "template-panel")
    
    # Technical panel
    if technical_data:
        content += create_data_panel("Technical Specification", technical_data, "technical-panel")
    ...
    # Create the full HTML page
    template = get_base_template()
    breadcrumb = create_breadcrumb("Element", section_title=section_title, element_name=element_name)
    html_content = template.format(
